...
/* application handle -> device number, plus the application's own callback,
 * so that the MOM_DONE notifications of the intermediate chunks never reach
 * the application, which expects exactly one per send of its own. */
#define MAXH 32
static struct {
    HMIDIOUT  h;
    UINT      dev;
    ...
    int       suppress;  /* swallow the MOM_DONE of an intermediate chunk */
} handles[MAXH];
                              ...
                              DWORD_PTR p1, DWORD_PTR p2)
{
    ...
    if (!handles[i].h) return;      /* slot already released: do not call it */
    if (msg == MOM_DONE && handles[i].suppress) return;
    if (handles[i].cb)
        ((MIDIOUTPROC)handles[i].cb)(h, msg, handles[i].inst, p1, p2);
}
...
MMRESULT WINAPI midiOutClose(HMIDIOUT h)
{
    ...
    EnterCriticalSection(&cs);
    for (int i = 0; i < MAXH; i++)
        if (handles[i].h == h) { ZeroMemory(&handles[i], sizeof(handles[i])); break; }
    LeaveCriticalSection(&cs);
    return r;
}

static int device_of(HMIDIOUT h, UINT *dev)
{
    int found = 0;
    EnterCriticalSection(&cs);
    for (int i = 0; i < MAXH; i++)
        if (handles[i].h == h) { *dev = handles[i].dev; found = 1; break; }
    LeaveCriticalSection(&cs);
    return found;
}
...
MMRESULT WINAPI midiOutLongMsg(HMIDIOUT h, LPMIDIHDR hdr, UINT size)
{
    ...
    /* The chunking reuses the application's own header: that way every pointer
     * that comes back to it is its own and is still valid. Wine services
     * MOM_DONE synchronously within this same call, so a single flag is enough
     * to swallow the notifications of the intermediate chunks. */
    char  *data  = hdr->lpData;
    DWORD  total = hdr->dwBufferLength;
    DWORD  rec   = hdr->dwBytesRecorded;
    int    slot  = slot_of(h);
    DWORD  off   = 0;
    unsigned pieces = 0;
    ...
    while (off < total) {
        ...
        if (slot >= 0) handles[slot].suppress = !last;

        hdr->lpData = data + off;
        hdr->dwBufferLength = n;
        hdr->dwBytesRecorded = n;
        ...
        off += n;
        pieces++;
        if (!last && chunk_delay) Sleep(chunk_delay);
    }

    if (slot >= 0) handles[slot].suppress = 0;

    hdr->lpData = data;
    hdr->dwBufferLength = total;
    hdr->dwBytesRecorded = rec;
